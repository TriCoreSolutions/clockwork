...
class EloquentDataSource extends DataSource
{
    ...
    private function resolveBindings($bindings, $query, $connection = null)
    {
        if ($this->databaseManager->connection($connection)->getPdo()->getAttribute(\PDO::ATTR_DRIVER_NAME) == 'odbc') {
            foreach ($bindings as $binding) {

                if (is_bool($binding)) {
                    $query = str_replace_first('?', $binding, $query);
                } else {
                    $query = str_replace_first('?', sprintf("'%s'", $binding), $query);
                }
            }
        } else {
            foreach ($bindings as $binding) {
    ...
    protected function createRunnableQuery($query, $bindings, $connection)
    {
        ...
        $query = $this->resolveBindings($bindings, $query, $connection);
